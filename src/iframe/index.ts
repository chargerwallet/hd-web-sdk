import HttpTransport from '@chargerwallet/hd-transport-http';
import WebusbTransport from '@chargerwallet/hd-transport-webusb';
import {
  PostMessageEvent,
  IFRAME,
  parseMessage,
...
const handleMessage = (event: PostMessageEvent) => {
  if (event.comurce === window || !event.data) return;
